...
> 适用场景：希望每个 Agent 有独立身份（名字+头像）、独立 API 配额或权限隔离。
> 背景说明见 [飞书接入指南 — 多应用模式](./FEISHU_SETUP.md#进阶为每个-agent-配置独立-bot多应用模式)。

在 `channels.feishu` 中使用 `accounts` 字段替代单一的 `appId`/`appSecret`：
...
> **注意**：使用多账户模式时，每个应用需要独立完成事件订阅和权限配置。`groups` allowlist 保持不变。
