...
| 文件 | 作用 | 更新频率 |
|------|------|---------|
| IDENTITY.md | 名字、emoji、一句话定位 | 极少 |
| SOUL.md | 角色指令、核心原则、自主边界（**优先级最高，Agent 启动必读**）| 低 |
| AGENTS.md | 工作流程、任务处理逻辑、spawn 规则 | 中 |
| USER.md | 用户画像：偏好、约束、沟通风格 | 低 |
| TOOLS.md | 工具和环境配置说明 | 低 |
| MEMORY.md | 长期记忆：稳定偏好、原则、经验 | 中 |
| TASKS.md | 当前任务看板 | 高 |
| HEARTBEAT.md | 周期性检查清单 | 按需 |

**关键设计**：SOUL.md 放在最前面读取，确保角色定位在 Agent 的上下文中优先级最高。

## Shared 协议说明

所有 Agent 共享的规则和模板放在 `~/.openclaw/shared/` 目录：

| 文件 | 用途 |
|------|------|
| SYSTEM_RULES.md | 全局系统规则（自主等级、任务分类、产物要求） |
| A2A_PROTOCOL.md | Agent 间协作协议（两步触发、权限矩阵） |
| OPS_REVIEW_PROTOCOL.md | 治理审计规则（审核维度、节奏） |
| KNOWLEDGE_PIPELINE.md | 知识沉淀管道（三层结构、KO 工作流） |
| TASK_PROTOCOL.md | 任务分类和处理协议 |
| CLOSEOUT_TEMPLATE.md | 任务收尾模板 |
| CHECKPOINT_TEMPLATE.md | 检查点模板 |
| SUBAGENT_PACKET_TEMPLATE.md | 子 Agent 任务包模板 |
| SELF_UPDATE_TEMPLATE.md | 自我更新记录模板 |

## 配置层硬约束

以下行为通过 `openclaw.json` 在配置层强制约束，不依赖 Agent "自觉遵守"：

| 约束 | 配置项 | 效果 |
|------|--------|------|
| A2A 发起权限 | `tools.agentToAgent.allow` | 只有 CoS/Ops/CTO 能发起 sessions_send |
| A2A 循环上限 | `session.agentToAgent.maxPingPongTurns` | 最多 4 次来回 |
| Subagent 权限 | `tools.subagents.tools.deny` | 子 Agent 不能操作 sessions |
| 频道隔离 | `channels.slack.groupPolicy = "allowlist"` | 只响应允许的频道 |
| Ops 噪音控制 | `requireMention = true`（可选） | 建议对 #ops/#know 开启 @mention gate（开源版默认关闭，跑通后再开） |
| Thread 隔离 | `thread.historyScope = "thread"` | 每个 thread 是独立 session |
| 回复模式 | `replyToMode = "all"` | 所有回复自动进入 thread |
