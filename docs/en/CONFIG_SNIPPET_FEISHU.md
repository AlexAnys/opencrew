...
### G) Multi-Account Config (Advanced: Multi-Bot Mode)

> Use case: you want each Agent to have its own identity (name + avatar), independent API quotas, or permission isolation.
> For background, see [Feishu Setup Guide -- Multi-App Mode](FEISHU_SETUP.md#advanced-separate-bot-per-agent-multi-app-mode).

In `channels.feishu`, use the `accounts` field instead of a single `appId`/`appSecret`:
    ...
    "feishu": {
      ...
      "accounts": {
        "cos-bot": {
          ...
          "appId": "cli_cos_xxxxx",
          "appSecret": "<COS_APP_SECRET>",
          "enabled": true
        },
        "cto-bot": {
          "name": "CTO Tech Partner",
          "appId": "cli_cto_xxxxx",
          "appSecret": "<CTO_APP_SECRET>",
          "enabled": true
        },
        "builder-bot": {
          "name": "Builder Executor",
          "appId": "cli_builder_xxxxx",
          "appSecret": "<BUILDER_APP_SECRET>",
          "enabled": true
        }
      }
    }
...
The corresponding `bindings` need an `accountId`:

```json
{
  "bindings": [
    { "agentId": "cos", "accountId": "cos-bot", "match": { "channel": "feishu", "peer": { "kind": "group", "id": "<FEISHU_GROUP_ID_HQ>" } } },
    { "agentId": "cto", "accountId": "cto-bot", "match": { "channel": "feishu", "peer": { "kind": "group", "id": "<FEISHU_GROUP_ID_CTO>" } } },
    { "agentId": "builder", "accountId": "builder-bot", "match": { "channel": "feishu", "peer": { "kind": "group", "id": "<FEISHU_GROUP_ID_BUILD>" } } }
  ]
}
```

> **Note**: When using multi-account mode, each app needs its own event subscriptions and permissions configured independently. The `groups` allowlist remains unchanged.
